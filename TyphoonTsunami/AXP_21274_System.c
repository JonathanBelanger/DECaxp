...
AXP_21274_SYSTEM *AXP_21274_AllocateSystem(void)
{
	...
	if (pthreadRet == 0)
	{
		...
		if (ii < sys->cpuCount)
		{
			...
			AXP_21264_Save_SystemInterfaces(cpu[ii],
			        ...
			        (void **) &sys->cpu[ii].pq, &sys->cpu[ii].pqTop,
	...
	if (qRet == true)
	{
	    ...
	    pthreadRet = pthread_create(&sys->cChipThreadID, NULL,
		    AXP_21274_CchipMain, sys);
	    if (pthreadRet == 0)
		pthreadRet = pthread_create(&sys->p0.threadID, NULL,
		        AXP_21274_PchipMain, sys);
	    if (pthreadRet == 0)
		pthreadRet = pthread_create(&sys->p1.threadID, NULL,
		        AXP_21274_PchipMain, sys);
	}
    }
