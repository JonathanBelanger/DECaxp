                       ...
                       AXP_PIPELINE *pipeline)
{
    ...
    {
        decodedInstr->type = AXP_DecodeOperType(decodedInstr->opcode,
    ...
    if (decodedInstr->decodedReg.bits.opcodeRegDecode != 0)
    {
        decodedInstr->decodedReg.raw =
            decodeFuncs[decodedInstr->decodedReg.bits.opcodeRegDecode](next->instructions[nextInstr]);
    }
 ...
 *  None.
 */
static void AXP_RenameRegisters(AXP_21264_CPU *cpu,
                                AXP_INSTRUCTION *decodedInstr)
{
    ...
    if (src1Float == false)
    {
        decodedInstr->aSrc1 = AXP_REG(decodedInstr->aSrc1,
                                      decodedInstr->pc.pal);
        src1Phys = cpu->pr;
        src1Map = cpu->prMap;
    }
    ...
    if (src2Float == false)
    {
        decodedInstr->aSrc2 = AXP_REG(decodedInstr->aSrc2,
                                      decodedInstr->pc.pal);
        src2Phys = cpu->pr;
        src2Map = cpu->prMap;
    }
    ...
    if (destFloat == false)
    {
        decodedInstr->aDest = AXP_REG(decodedInstr->aDest,
                                      decodedInstr->pc.pal);
        destPhys = cpu->pr;
        destMap = cpu->prMap;
    ...
    else
    {
        ...
        if (destPhys[destMap[decodedInstr->aDest]].refCount == 0)
        {
            ...
            {
                AXP_TRACE_BEGIN();
                AXP_TraceWrite("AXP_RenameRegisters freeing P%c%02d back onto "
                               "the p%cFreeList[%u]",
                               (destFloat ? 'F' : 'R'),
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("AXP_RenameRegisters mapping %c%02d --> P%c%02d "
                           "from p%cFreeList[%u]",
                           (destFloat ? 'F' : 'R'),
                           decodedInstr->aDest,
                           (destFloat ? 'F' : 'R'),
    ...
    if (AXP_IBOX_OPT1)
    {
        ...
        AXP_TraceWrite("AXP_RenameRegisters returning for pc: 0x%016llx, "
                       "with mapping of:",
                       decodedInstr->pc);
        AXP_TraceWrite("\t%c%02u --> P%c%02u (%s)",
                       (src1Float ? 'F' : 'R'),
                       decodedInstr->aSrc1,
                       (src1Float ? 'F' : 'R'),
                       decodedInstr->src1,
...
u32 AXP_UpdateRegisters(AXP_21264_CPU *cpu, AXP_INSTRUCTION *instr)
{
    ...
    if (instr->aDest != AXP_UNMAPPED_REG)
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("AXP_UpdateRegisters saving %c%02d (P%c%02d) = "
                           "0x%016llx",
                           (destFloat ? 'F' : 'R'),
                           instr->aDest,
                           (destFloat ? 'F' : 'R'),
        ...
        (src1Phys[instr->src1].refCount == 0))
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("AXP_UpdateRegisters freeing P%c%02d back onto the "
                           "p%cFreeList[%u] : (src1)",
                           (src1Float ? 'F' : 'R'),
                           instr->src1,
                           (src1Float ? 'f' : 'r'),
...
bool AXP_AbortInstructions(AXP_21264_CPU *cpu, AXP_INSTRUCTION *inst)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_AbortInstructions called robStart = %u : "
                       "robEnd = %u",
                       cpu->robStart,
                       cpu->robEnd);
        AXP_TRACE_END();
    ...
    while ((endIdx != cpu->robStart) &&
           (AXP_GET_PC(rob->pc) != AXP_GET_PC(inst->pc)))
    {
        ...
        if (rollbackRegisterMap == true)
        {
            ...
            */
            if (rob->dest != AXP_UNMAPPED_REG)
            {
                ...
                {
                    AXP_TRACE_BEGIN();
                    AXP_TraceWrite("AXP_AbortInstructions freeing P%c%02d back "
                                   "onto the p%cFreeList[%u]",
                                   rob->src1,
                                   ((destPhys == cpu->pf) ? 'f' : 'r'),
                                   *destFlStart);
                    AXP_TRACE_END();
...
void AXP_RegisterRename_IntegrityCheck(AXP_21264_CPU *cpu)
{
    ...
    for (ii = 0; ii < AXP_INT_PHYS_REG; ii++)
    {
        if (physInUse[ii] != 1)
        {
            ...
            {
                AXP_TRACE_BEGIN();
                AXP_TraceWrite(">>>> Physical Register PR%02d is referenced "
                               "%u times.",
                               ii,
                               physInUse[ii]);
                AXP_TRACE_END();
    ...
    for (ii = 0; ii < AXP_FP_PHYS_REG; ii++)
    {
        if (physInUse[ii] != 1)
        {
            ...
            {
                AXP_TRACE_BEGIN();
                AXP_TraceWrite(">>>> Physical Register PF%02d is referenced "
                               "%u times.",
                               ii,
                               physInUse[ii]);
                AXP_TRACE_END();
