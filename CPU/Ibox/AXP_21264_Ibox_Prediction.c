 ...
 */
bool AXP_Branch_Prediction(AXP_21264_CPU *cpu,
                           AXP_PC vpc,
                           bool *localTaken,
                           bool *globalTaken,
                           bool *choice)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("Called into AXP_Branch_Prediction for pc: 0x%016llx",
                       AXP_GET_PC(vpc));
        AXP_TRACE_END();
    }
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("Returning (%d) from AXP_Branch_Prediction for pc: "
                       "0x%016llx ,local taken = %d, global taken = %d, "
                       "choice = %d",
                       retVal,
                       AXP_GET_PC(vpc),
                       *localTaken,
 ...
 *  None.
 */
void AXP_Branch_Direction(AXP_21264_CPU *cpu,
                          AXP_PC vpc,
                          bool taken,
                          bool localTaken,
                          bool globalTaken)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("Called into AXP_Branch_Direction for pc: 0x%016llx",
                       AXP_GET_PC(vpc));
        AXP_TRACE_END();
    }
    ...
    else
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("AXP_Branch_Direction for pc: 0x%016llx, Branch "
                           "Not Taken",
                           AXP_GET_PC(vpc));
            AXP_TRACE_END();
        }
