/*
 * Copyright (C) Jonathan D. Belanger 2017-10`9.
 * All Rights Reserved.
 *
...
OP_ITOFS: /* OPCODE: 0x14 */
    fpFunc.tmp1 = instr->function;
    goto *(&&FUNC_ITOF + opcode14Label[fpFunc.tmp2.fnc]);

FUNC_ITOF: /* FNC: 0x4 */
    switch (fpFunc.tmp2.src)
    {
        case AXP_FP_S:
...
OP_ADDF: /* OPCODE: 0x15 */
    fpFunc.tmp1 = instr->function;
    goto *(&&FUNC_VAX_ADD + opcode15Label[fpFunc.tmp2.fnc]);
...
OP_ADDS: /* OPCODE: 0x16 */
    fpFunc.tmp1 = instr->function;
    goto *(&&FUNC_IEEE_ADD + opcode16Label[fpFunc.tmp2.fnc]);
...
AXP_INS_TYPE AXP_InstructionFormat(AXP_INS_FMT inst)
{
    ...
    if (inst.pal.opcode <= 0x3f)
    {
        ...
        if (retVal == Cond)
        {
            retVal = ((inst.fp.func == 0x70) || (inst.fp.func == 0x78)) ?
                        FP :
                        Opr;
        }
    }
