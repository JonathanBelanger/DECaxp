 ...
 *
 *  V01.015 19-May-2019 Jonathan D. Belanger
 ...
 *  None.
 */
void AXP_21264_Ibox_Event(AXP_21264_CPU *cpu,
                          u32 fault,
                          AXP_PC pc,
                          u64 va,
                          u8 opcode,
                          ...
                          bool self)
{
    ...
    if (cpu->excPend == false)
    {
         ...
         */
        cpu->va = 0;
        AXP_IBOX_WRITE_EXC_SUM(0, cpu);
        AXP_MBOX_WRITE_MM_STAT(0, cpu);
 ...
 *  None.
 */
void AXP_21264_Ibox_UpdateIcache(AXP_21264_CPU *cpu,
                                 u64 pa,
                                 u8 *data,
                                 bool dontSignal)
{
     ...
     */
    AXP_PUT_PC(pc, va);
    AXP_IcacheAdd(cpu, pc, (u32 *) data, itb);
         ...
         (instr->type_hint_index <= AXP_IPR_PCXT1_FPE_PPCE_ASTRR_ASTER_ASN)))
    {
        ...
        switch (instr->type_hint_index)
        {
            ...
            case AXP_IPR_ITB_PTE:
                ...
                */
                AXP_IBOX_READ_ITB_TAG(tbTag, cpu);
                AXP_IBOX_READ_ITB_PTE(tbPte, cpu);
                AXP_addTLBEntry(cpu, tbTag, tbPte, false);
                break;
            ...
            case AXP_IPR_ITB_IS:
                 ...
                 * ITB_IS IPR.
                 */
                AXP_IBOX_READ_ITB_IS(tbIs, cpu);
                AXP_tbis(cpu, tbIs, false);
                break;
              ...
              (instr->type_hint_index <= AXP_IPR_DTB_ASN1)))
    {
        ...
        switch (instr->type_hint_index)
        {
            ...
            case AXP_IPR_DTB_PTE0:
                ...
                */
                AXP_MBOX_READ_DTB_TAG0(tbTag, cpu);
                AXP_MBOX_READ_DTB_PTE0(tbPte, cpu);
                AXP_addTLBEntry(cpu, tbTag, tbPte, true);
                break;
            ...
            case AXP_IPR_DTB_IS0:
                ...
                * DTB_IS0 IPR.
                */
                AXP_MBOX_READ_DTB_IS0(tbIs, cpu);
                AXP_tbis(cpu, tbIs, true);
                break;
            ...
            case AXP_IPR_DTB_PTE1:
                 ...
                 * written into the DTB entry.
                 */
                AXP_MBOX_READ_DTB_TAG1(tbTag, cpu);
                AXP_MBOX_READ_DTB_PTE1(tbPte, cpu);
                AXP_addTLBEntry(cpu, tbTag, tbPte, true);
                break;
            ...
            case AXP_IPR_DTB_IS1:
                 ...
                 * DTB_IS0 IPR.
                 */
                AXP_MBOX_READ_DTB_IS1(tbIs, cpu);
                AXP_tbis(cpu, tbIs, true);
                break;
...
bool AXP_21264_Ibox_Retire(AXP_21264_CPU *cpu)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_Ibox_Retire called (start: %u, end: %u)",
                       cpu->robStart,
                       cpu->robEnd);
        AXP_TRACE_END();
    }
    ...
    while ((ii < end) && (done == false))
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("ROB[%u] instruction at pc: 0x%016llx, "
        ...
        if (rob->state == WaitingRetirement)
        {
            ...
            else
            {
                ...
                if (rob->type == Branch)
                {
                    bool taken = AXP_GET_PC(rob->branchPC) != 0;
                    ...
                    if (taken != rob->branchPredict)
                    {
                        ...
                        {
                            AXP_TRACE_BEGIN();
                            AXP_TraceWrite("Branch MISPREDICT instruction at "
...
void *AXP_21264_IboxMain(void *voidPtr)
{
    ...
    while ((cpu->cpuState != Run) && (cpu->cpuState != ShuttingDown))
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("Ibox is waiting for CPU to be in Run State (%d)",
                           cpu->cpuState);
            AXP_TRACE_END();
        }
    ...
    while (cpu->cpuState == Run)
    {
        ...
        if (AXP_IcacheFetch(cpu, nextPC, &nextCacheLine) == true)
        {
            aborting = false;
            for (ii = 0;
                 ((ii < AXP_NUM_FETCH_INS) && (aborting == false));
                 ii++)
            {
                ...
                {
                    AXP_TRACE_BEGIN();
                    AXP_TraceWrite("ROB[%u] getting instruction at pc: "
                                   "0x%016llx",
                                   cpu->robEnd,
                                   AXP_GET_PC(nextPC));
                    AXP_TRACE_END();
                ...
                {
                    decodedInstr->branchPredict = AXP_Branch_Prediction(cpu,
                                                                        nextPC,
                ...
                {
                    AXP_TRACE_BEGIN();
                    AXP_TraceWrite("opcode: 0x%02x, index = 0x%02x, "
                                   "src1 = %02u, src2 = %02u, dest = %02u, "
                                   "pipeline = %d, NO_OP = %d",
                                   decodedInstr->opcode,
                ...
                if (noop == false)
                {
                    ...
                    if (whichQueue == AXP_IQ)
                    {
                        ...
                        AXP_InsertCountedQueue((AXP_CQUE_ENTRY *) &cpu->iq,
                    ...
                    else /* FQ */
                    {
                        ...
                        AXP_InsertCountedQueue((AXP_CQUE_ENTRY *) &cpu->fq,
                                               (AXP_CQUE_ENTRY *) xqEntry);
        ...
        else
        {
            ...
            itb = AXP_findTLBEntry(cpu, AXP_GET_PC(nextPC), false);
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("Ibox is not/no longer in the Run State (%d)",
                       cpu->cpuState);
        AXP_TRACE_END();
    }
