...
int AXP_21264_MAF_Empty(AXP_21264_CPU *cpu)
{
    int retVal = -1;
    int ii;
    int end, start1, start2 = -1, end1, end2 = 0;
 ...
 *      system.
 */
void AXP_21264_Complete_MAF(AXP_21264_CPU *cpu,
                            int entry,
                            AXP_SYSDC sysDc,
                            u8 *sysData)
{
    ...
    int dataLen;
    int ii = 0;
    u8 cacheStatus = AXP_21264_CACHE_MISS;
    ...
    switch (maf->type)
    {
        ...
        case Istream:
            ...
            if (maf->ioReq == false)
            {
                ...
                while (curPtr != -1)
                {
                    ...
                    {
                        AXP_21264_Mbox_UpdateDcache(cpu,
                                                    maf->lqSqEntry[ii],
                                                    &sysData[bufIndex],
                                                    cacheStatus);
                    }
        ...
        case MemoryBarrier:
            ...
            /* sysDc = MBDone */
            AXP_21264_Mbox_CboxCompl(cpu, maf->lqSqEntry[0], NULL, 0, false);
            break;
    }
 ...
 */
bool AXP_21264_Add_MAF_Mem(AXP_21264_CPU *cpu,
 ...
 *          existing one.  Nothing else to do.
 */
bool AXP_21264_Add_MAF_IO(AXP_21264_CPU *cpu,
                          AXP_CBOX_MAF_TYPE type,
                          u64 pa,
                          i8 lqSqEntry,
                          int dataLen,
                          bool shared)
{
    ...
    while ((ii <= end) && (maf == NULL))
    {
            ...
            (cpu->maf[ii].complete == false))
        {
            ...
            {
                paEnd = cpu->maf[ii].pa + cpu->maf[ii].bufLen;
                if ((paEnd <= pa) && ((pa + dataLen) <=
                    (cpu->maf[ii].pa + maxLen)))
                {
                    maf = &cpu->maf[ii];
                }
            }
 ...
 *  None.
 */
void AXP_21264_Add_MAF(AXP_21264_CPU *cpu,
                       AXP_CBOX_MAF_TYPE type,
                       u64 pa,
                       i8 lqSqEntry,
                       int dataLen,
                       bool shared)
{
    ...
    if (ioRq == false)
    {
        ...
        {
            notMerged = AXP_21264_Add_MAF_Mem(cpu,
                                              type,
                                              pa,
                                              lqSqEntry,
                                              dataLen,
    ...
    else
    {
        ...
        {
            notMerged = AXP_21264_Add_MAF_IO(cpu,
                                             type,
                                             pa,
                                             lqSqEntry,
                                             dataLen,
