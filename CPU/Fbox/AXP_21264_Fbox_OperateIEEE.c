...
#include "CPU/Fbox/AXP_21264_Fbox_OperateIEEE.h"
...
AXP_EXCEPTIONS AXP_ADDT(AXP_21264_CPU *cpu, AXP_INSTRUCTION *instr)
{
    ...
    if (raised == 0)
    {
         ...
         */
        src1v.tmp1 = instr->src1v.fp.uq;
        src2v.tmp1 = instr->src2v.fp.uq;
         ...
         * Execute the instruction.
         */
        destv.tmp2 = src1v.tmp2 + src2v.tmp2;
...
AXP_EXCEPTIONS AXP_FTOIS(AXP_21264_CPU *cpu, AXP_INSTRUCTION *instr)
{
     ...
     * Execute the instruction.
     */
    instr->destv.r.uq = ((instr->src1v.fp.fpr32.sign == 1 ?
                          0xffffffff80000000ll : 0) |
...
AXP_EXCEPTIONS AXP_ITOFS(AXP_21264_CPU *cpu, AXP_INSTRUCTION *instr)
{
    union
    {
        u32 tmp1;
        float tmp2;
    } src1v = { .tmp1 = instr->src1v.r.ul };

    /*
     * Execute the instruction.
     */
    instr->destv.fp = AXP_FP_CvtFloatToFPR(src1v.tmp2);
