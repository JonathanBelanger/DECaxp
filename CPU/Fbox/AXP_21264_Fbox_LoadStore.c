 ...
 *
 *  V01.003 18-May-2019 Jonathan D. Belanger
...
#include "CPU/Fbox/AXP_21264_Fbox_LoadStore.h"
...
AXP_EXCEPTIONS AXP_STF(AXP_21264_CPU *cpu, AXP_INSTRUCTION *instr)
{
     ...
     * 32-bits.
     */
    memFmt.tmpF.sign = instr->src1v.fp.fCvt.sign;
    memFmt.tmpF.exponent = exp;
    memFmt.tmpF.fractionHigh = instr->src1v.fp.fCvt.fractionHigh;
    memFmt.tmpF.fractionLow = instr->src1v.fp.fCvt.fractionLow;

    AXP_21264_Mbox_WriteMem(cpu, instr, instr->slot, vaPrime, memFmt.tmp);
...
AXP_EXCEPTIONS AXP_STG(AXP_21264_CPU *cpu, AXP_INSTRUCTION *instr)
{
     ...
     * 64-bits.
     */
    memFmt.tmpG.sign = instr->src1v.fp.gCvt.sign;
    memFmt.tmpG.exponent = instr->src1v.fp.gCvt.exponent;
    memFmt.tmpG.fractionHigh = instr->src1v.fp.gCvt.fractionHigh;
    memFmt.tmpG.fractionMidHigh = instr->src1v.fp.gCvt.fractionMidHigh;
    memFmt.tmpG.fractionMidLow = instr->src1v.fp.gCvt.fractionMidLow;
    memFmt.tmpG.fractionLow = instr->src1v.fp.gCvt.fractionLow;

    AXP_21264_Mbox_WriteMem(cpu, instr, instr->slot, va, memFmt.tmp);
...
AXP_EXCEPTIONS AXP_STS(AXP_21264_CPU *cpu, AXP_INSTRUCTION *instr)
{
     ...
     * 32-bits.
     */
    memFmt.tmpS.sign = instr->src1v.fp.sCvt.sign;
    memFmt.tmpS.exponent = exp;
    memFmt.tmpS.fraction = instr->src1v.fp.sCvt.fraction;
