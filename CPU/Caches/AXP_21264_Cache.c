...
AXP_21264_TLB *AXP_findTLBEntry(AXP_21264_CPU *cpu, u64 virtAddr, bool dtb)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_findTLBEntry for address 0x%016llx in %s called.",
                       virtAddr,
                       (dtb ? "DTB" : "ITB"));
        AXP_TRACE_END();
    }
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_findTLBEntry returning %s at 0x%016llx.",
                       (dtb ? "DTB" : "ITB"),
                       (u64 *) retVal);
        AXP_TRACE_END();
    }
...
AXP_21264_TLB *AXP_getNextFreeTLB(AXP_21264_TLB *tlbArray, u32 *nextTLB)
{
    ...
    *nextTLB = (*nextTLB >= (AXP_TB_LEN - 1) ? 0 : *nextTLB + 1);
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_getNextFreeTLB returning 0x%016llx",
                       (u64 *) retVal);
        AXP_TRACE_END();
    }
...
void AXP_addTLBEntry(AXP_21264_CPU *cpu, u64 virtAddr, u64 physAddr, bool dtb)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_addTLBEntry for address (va: 0x%016llx, pa: 0x%016llx) to %s called.",
                       virtAddr,
                       physAddr,
                       (dtb ? "DTB" : "ITB"));
        AXP_TRACE_END();
...
void AXP_tbia(AXP_21264_CPU *cpu, bool dtb)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_tbia to invalidate all %s entries called.",
                       (dtb ? "DTB" : "ITB"));
        AXP_TRACE_END();
    }
...
void AXP_tbiap(AXP_21264_CPU *cpu, bool dtb)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_tbiap to invalidate all process specific %s entries called.",
                       (dtb ? "DTB" : "ITB"));
        AXP_TRACE_END();
    }
...
void AXP_tbis(AXP_21264_CPU *cpu, u64 va, bool dtb)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_tbis to invalidate a single %s entry for 0x%016llx called.",
                       (dtb ? "DTB" : "ITB"),
                       va);
        AXP_TRACE_END();
    }
 ...
 *                  is not valid.
 */
AXP_EXCEPTIONS AXP_21264_checkMemoryAccess(AXP_21264_CPU *cpu,
                                           AXP_21264_TLB *tlb,
                                           AXP_21264_ACCESS acc)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_21264_checkMemoryAccess tlb: 0x%016llx, access: %s called.",
                       (u64 *) tlb,
                       accStr[acc]);
        AXP_TRACE_END();
    }
 ...
 */
u64 AXP_va2pa(AXP_21264_CPU *cpu,
              u64 va,
              AXP_PC pc,
              bool dtb,
              AXP_21264_ACCESS acc,
              ...
              AXP_EXCEPTIONS *memChk)
{
    ...
    *memChk = NoException;
    ...
    else if ((spe != 0) && (cpu->ierCm.cm == AXP_CM_KERNEL))
    {
        ...
        {
            pa = ((va & AXP_SPE1_VA_MASK) | ((va & AXP_SPE1_VA_40) ?
                AXP_SPE1_PA_43_41 :
                0));
            if (_asm != NULL)
            {
 ...
 *          mutex.
 */
AXP_EXCEPTIONS AXP_Dcache_Status(AXP_21264_CPU *cpu,
                                 u64 va,
                                 u64 pa,
                                 u32 len,
    ...
    switch (len)
    {
        ...
        case 64:
            ...
            else
            {
                ...
                if (AXP_CACHE_CLEAN_SHARED(status) ||
                    AXP_CACHE_DIRTY_SHARED(status))
                {
                    dtag->shared = true;
                }
                else
 ...
 */
void AXP_CopyBcacheToDcache(AXP_21264_CPU *cpu,
                            AXP_DCACHE_LOC *indexSetOffset,
                            u64 pa)
{
    u32 index = indexSetOffset->index;
 ...
 *  None.
 */
void AXP_Dcache_Lock(AXP_21264_CPU *cpu, u64 va, u64 pa)
{
    AXP_VA virtAddr = {.va = va};
    AXP_VA physAddr = {.va = pa};
    u64 tag = physAddr.vaIdxInfo.tag;
                    ...
                    AXP_DCACHE_LOC *idxSet)
{
    ...
    for (ii = 0; ii < sets; ii++)
    {
        if ((cpu->dtag[virtAddr.vaIdx.index][ii].valid == true) &&
            (cpu->dtag[virtAddr.vaIdx.index][ii].physTag == physAddr.vaIdxInfo.tag))
        {
                   ...
                   AXP_21264_TLB *itb)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_IcacheAdd added at iCache[%u][%u].",
                       index,
                       whichSet);
        AXP_TRACE_END();
    }
    ...
    for (ii = 0; ii < AXP_CACHE_ENTRIES; ii++)
    {
        ...
        if (cpu->iCache[ii][0].vb == 1)
        {
                ...
                (purgeAsm == false))
            {
                ...
                {
                    memset(&cpu->iCache[ii][0].instructions[jj],
                           0,
                           sizeof(AXP_INS_FMT));
                }
            }
        ...
        if (cpu->iCache[ii][1].vb == 1)
        {
                ...
                (purgeAsm == false))
            {
                ...
                for (jj = 0; jj < AXP_ICACHE_LINE_INS; jj++)
                {
                    memset(&cpu->iCache[ii][1].instructions[jj],
                           0,
                           sizeof(AXP_INS_FMT));
                }
            }
...
bool AXP_IcacheFetch(AXP_21264_CPU *cpu, AXP_PC pc, AXP_INS_LINE *next)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_IcacheFetch returning iCache[%u][%u] with status %d.",
                       index,
                       whichSet,
                       retVal);
        AXP_TRACE_END();
