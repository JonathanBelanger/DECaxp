...
static const size_t _head_tail_size = sizeof(AXP_BLOCK_HD) + sizeof(AXP_BLOCK_TL);
...
void AXP_Deallocate_Block(void *block)
{
        ...
        (tail->magicNumber == AXP_TL_MAGIC))
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("AXP_Deallocate_Block deallocating %s of size %d at 0x%016llx",
...
AXP_BLOCK_TYPE AXP_ReturnType_Block(void *block)
{
    ...
    if (block != NULL)
    {
        ...
        if ((head->head.flink != NULL) &&
            (head->head.blink != NULL) &&
            (head->type == tail->type) &&
            ((head->type > AXP_UNKNOWN_BLK) &&
             (head->type < AXP_BLOCK_MAX)) &&
            (head->size == tail->size) &&
