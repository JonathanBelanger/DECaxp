...
static bool AXP_RegistersReady(AXP_21264_CPU *cpu, AXP_QUEUE_ENTRY *entry)
{
  ...
  AXP_TraceWrite(
          "AXP_RegistersReady checking registers at pc = 0x%016llx, "
            "opcode = 0x%02x:", *((u64 *) &entry->ins->pc),
          (u32) entry->ins->opcode);
  AXP_TraceWrite("\tSrc1 (%c%02u) = %s", (src1Float ? 'F' : 'R'),
          entry->ins->aSrc1, regStateStr[src1Reg[entry->ins->src1].state],
          (src1Float ? 'F' : 'R'), entry->ins->src1);
  AXP_TraceWrite("\tSrc2 (%c%02u) = %s", (src2Float ? 'F' : 'R'),
          entry->ins->aSrc2, regStateStr[src2Reg[entry->ins->src2].state],
          (src2Float ? 'F' : 'R'), entry->ins->src2);
  AXP_TraceWrite("\tDest (%c%02u) = %s (P%c%02u) %s",
          (destFloat ? 'F' : 'R'), entry->ins->aDest,
          regStateStr[destReg[entry->ins->dest].state],
          (destFloat ? 'F' : 'R'), entry->ins->dest, warn);
  AXP_TRACE_END()
  ;
    }
 ...
 *	None.
 */
void AXP_Execution_Box(AXP_21264_CPU *cpu, AXP_PIPELINE pipeline,
        AXP_COUNTED_QUEUE *queue, pthread_cond_t *cond, pthread_mutex_t *mutex,
        void (*returnEntry)(AXP_21264_CPU *, AXP_QUEUE_ENTRY *))
{
                  ...
                  || (nothingReadyForMe == true)))
  {
      ...
      {
    AXP_TRACE_BEGIN();
    AXP_TraceWrite("%s signaled [%s] = %u.", pipelineStr[pipeline],
            (eBox ? eBoxClusterStr[clusterCountIdx] : fBoxClusterStr[clusterCountIdx]),
  ...
  if (cpu->cpuState != ShuttingDown)
  {
    ...
    if (AXP_UTL_OPT2)
    {
        ...
        AXP_TraceWrite("%s queue = 0x%016llx, entry = 0x%016llx, "
          "next = 0x%016llx", pipelineStr[pipeline], queue,
          entry, next);
        AXP_TraceWrite("%s checking at "
          "pc = 0x%016llx, "
          "opcode = 0x%02x, "
          "pipeline = %s, "
          "state = %s.", pipelineStr[pipeline],
          *((u64 *) &entry->ins->pc),
          (u32) entry->ins->opcode,
    ...
    if ((entry->pipeline == EboxU0) || (entry->pipeline == EboxU0U1)
            || (entry->pipeline == EboxL0L1U0U1))
        cpu->eBoxClusterCounter[AXP_21264_EBOX_U0]--;
    if ((entry->pipeline == EboxU1) || (entry->pipeline == EboxU0U1)
            || (entry->pipeline == EboxL0L1U0U1))
        cpu->eBoxClusterCounter[AXP_21264_EBOX_U1]--;
    if ((entry->pipeline == EboxL0) || (entry->pipeline == EboxL0L1)
            || (entry->pipeline == EboxL0L1U0U1))
        cpu->eBoxClusterCounter[AXP_21264_EBOX_L0]--;
    if ((entry->pipeline == EboxL1) || (entry->pipeline == EboxL0L1)
            || (entry->pipeline == EboxL0L1U0U1))
    ...
    AXP_TraceWrite("%s has something to process at "
      "pc = 0x%016llx, opcode = 0x%02x.",
            pipelineStr[pipeline], *((u64 *) &entry->ins->pc),
            (u32) entry->ins->opcode);
    AXP_TRACE_END()
    ;
      }
    ...
    if (AXP_UTL_OPT2)
    {
        ...
        AXP_TraceWrite(
          ...
          pipelineStr[pipeline], entry->ins->opcode);
        AXP_TRACE_END()
        ;
    }
    ...
    if (AXP_UTL_OPT2)
    {
        ...
        AXP_TraceWrite("Fbox %s : Floating point instructions are "
          "currently disabled.", pipelineStr[pipeline]);
        AXP_TRACE_END()
        ;
    }
    pthread_mutex_lock(&cpu->robMutex);
