 ...
 *  It is also reporting potentially uninitialized variables where it did not
 *  previously.  That is what occurred in this module.
 */
...
void AXP_LRUAdd(AXP_QUEUE_HDR *lruQ, AXP_QUEUE_HDR *entry)
{
    ...
    if (entry->flink != (void *) &lruQ->flink)
    {
         ...
         * queue, so remove us from the queue.
         */
...
bool AXP_VerifyCountedQueue(AXP_COUNTED_QUEUE *parent)
{
    ...
    if (AXP_UTL_BUFF)
    {
        ...
        AXP_TraceWrite("Checking integrity of counted queue 0x%016llx, "
    ...
    while (((void *) next != (void *)parent) && (entries < parent->max))
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("\t0x%016llx --> 0x%016llx",
                           (u64) next,
                           (u64) next->flink);
            AXP_TRACE_END();
        }
        ...
        (entries == parent->count))
    {
        ...
        {
            AXP_TRACE_BEGIN();
            AXP_TraceWrite("Checking integrity of blink for 0x%016llx",
                           (u64) parent);
            AXP_TRACE_END();
        }
        next = parent->blink;
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("Queue @ 0x%016llx integrity is %s",
                       (u64) parent,
                       (retVal ? "GOOD" : "BAD"));
        AXP_TraceWrite("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
        AXP_TRACE_END();
...
i32 AXP_InsertCountedQueue(AXP_CQUE_ENTRY *pred, AXP_CQUE_ENTRY *entry)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_InsertCountedQueue Called for 0x%016llx to "
                       "insert 0x%016llx",
                       (u64) entry->parent,
                       (u64) entry);
        AXP_TRACE_END();
...
i32 AXP_CountedQueueFull(AXP_COUNTED_QUEUE *parent, u32 headRoom)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_CountedQueueFull Called for 0x%016llx to "
                       "with head-room of %u",
                       (u64) parent,
                       headRoom);
        AXP_TRACE_END();
...
i32 AXP_RemoveCountedQueue(AXP_CQUE_ENTRY *entry, bool locked)
{
    ...
    {
        AXP_TRACE_BEGIN();
        AXP_TraceWrite("AXP_RemoveCountedQueue Called for 0x%016llx to "
                       "remove 0x%016llx",
                       (u64) entry->parent,
                       (u64) entry);
        AXP_TRACE_END();
...
i32 AXP_CondQueue_Insert(AXP_COND_Q_LEAF *where, AXP_COND_Q_LEAF *what)
{
    ...
    if (pthread_mutex_lock(&parent->qMutex) == 0)
    {
         ...
         * the new entry into the existing queue at the location indicated.
         * We'll also need to increment the counter, if there is one, and
         * broadcast the condition variable.
         */
        if (retVal == 0)
        {
            what->flink = where->flink;
            what->blink = where;
            where->flink = what;
            what->flink->blink = what;
 ...
 *  read.
 *
 *      NOTE:   We only ever read in an executable file and never write one.
 *              So, there is no companion Unload function.
 *
 * Input Parameters:
 *  fineName:
...
i32 AXP_LoadExecutable(char *fileName, u8 *buffer, u32 bufferLen)
{
    ...
    if (fp != NULL)
    {
        ...
        /*
         * If we did not detect an error, then return the number of bytes read.
         */
        if (retVal == 0)
        {
            retVal = ii;
...
bool AXP_OpenRead_SROM(char *fileName, AXP_SROM_HANDLE *sromHandle)
{
    ...
    if (sromHandle->fp != NULL)
    {
         ...
         */
        if ((retVal == false) &&
            (AXP_Crc32((u8 *) &sromHandle->validPat,
                       AXP_ROM_HDR_LEN,
                       false,
 ...
 *  true:   An error occurred.
 */
bool AXP_OpenWrite_SROM(char *fileName,
    ...
    if (feof(sromHandle->fp) == 0)
    {
        ...
        if (feof(sromHandle->fp) != 0)
        {
            ...
            if (sromHandle->verImgChecksum != sromHandle->imgChecksum)
            {
                ...
                {
                    AXP_TRACE_BEGIN();
                    AXP_TraceWrite("Read Image Checksum 0x%08x does not match "
                                   "read in Checksum 0x%08x",
                                   sromHandle->verImgChecksum,
                                   sromHandle->imgChecksum);
                    AXP_TRACE_END();
                }
...
int AXP_MaskGet(int *curPtr, u8 mask, int len)
{
    ...
    switch (len)
    {
        ...
        case CACHE_LEN:
        default:    /* This is to just keep the compiler from complaining */
 ...
 */
i32 AXP_Ascii2UTF_16(char *inBuf,
                     size_t inLen,
                     uint16_t *outBuf,
                     size_t *outLen)
{
 ...
 *  EBADF:      The cp argument is not a valid open conversion descriptor.
 */
i32 AXP_UTF16_2Ascii(uint16_t *inBuf,
                     size_t inLen,
                     char *outBuf,
                     size_t *outLen)
{
