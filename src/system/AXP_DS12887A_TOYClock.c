...
static void AXP_DS12887A_Normalize(struct tm *timeSpec, bool justTime)
{
	...
	timeSpec->tm_year += ((timeSpec->tm_year > 70) ? 100 : 0);
	mktime(timeSpec);
	timeSpec->tm_year %= 100;
    }
...
static void AXP_DS12887A_StartTimers(bool all)
{
	...
	/*
	 * Normalize the time, then get tne absolute time for the next trigger.
	 */
...
void AXP_DS12887A_Read(u8 addr, u8 *value)
{
	...
	case AXP_ADDR_Day:
	    retDay->bin.day = binTime.tm_wday + 1;
	    retDay->bin.res = 0;
	    break;
