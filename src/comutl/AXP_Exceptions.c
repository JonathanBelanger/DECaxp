...
/*
 * AXP_SetIEEEException
 * 	This function will be utilized throughout the code to set the IEEE
 * 	exception bits in the Instruction structure.  When the instruction is
 * 	retired, this information will be used to set the Floating-point Control
 * 	Register (fpcr).  If another instruction that generated an exception
 * 	retires before the one we are currently processing, then this information
 * 	will be flushed along with any subsequent set of instructions.
 *
 * Input Parameters:
 * 	instr:
 * 		A pointer to a structure containing the information needed to report
 * 		the exception upon instruction retirement.
 * 	exception:
 * 		A value indicating the bit fields to set in the FPCR.
 *
 * Output Parameters:
 * 	instr:
 * 		The fpcr will be updated, based on the exception.
 *
 * Return Value:
 * 	None.
 */
void AXP_SetIEEEException(AXP_INSTRUCTION *instr, u64 exception)
{
	u64	*fpcr = (u64 *) &instr->insFpcr;

	/*
	 * Set the FPCR.  NOTE: At this point we only care about the exception
	 * bits.  The enable/disable bits are only relevant in the real FPCR.
	 */
	*fpcr = exception;
	if (exception != 0)
		instr->insFpcr.sum = 1;
