 ...
 *
 * $Id: telnet.c,v 1.2 2002/10/07 22:28:15 scribe Exp $
 ***************************************************************************/
...
#include <time.h>
#include <io.h>
#else		/* defined(WIN32) && !defined(__GNUC__) || defined(__MINGW32__) */
...
#endif		/* else defined(WIN32) && !defined(__GNUC__) || defined(__MINGW32__) */
...
static void telrcv(
		struct connectdata *,
		unsigned char *,	/* Data received from socket */
...
static void send_negotiation(struct connectdata *, int, int);
static void set_local_option(struct connectdata *, int, int);
static void set_remote_option(struct connectdata *, int, int);
		...
		switch(waitret - WAIT_OBJECT_0)
		{
			...
			case 1:
					...
					SOCKET_ERROR)
				{
					...
					{
						/* This really OUGHT to check its return code. */
		...
		switch (select (sockfd + 1, &readfd, NULL, NULL, NULL))
		{
			...
			default:			/* read! */
				...
				if(FD_ISSET(sockfd, &readfd))
				{
					...
					/*
					 * if we receive 0 or less here, the server closed the
					 * connection and we bail out from this!
					 */
					if (nread <= 0)
					{
						keepon = FALSE;
