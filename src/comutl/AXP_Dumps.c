 ...
 *	None.
 */
void AXP_Decode_Instruction(AXP_PC *pcAddr, AXP_INS_FMT instr, bool kernelMode,
        char *instrStr)
{
	...
	case INTL:
		...
		switch (instr.oper1.func)
		{
			...
			strLoc += sprintf(&regStr[strLoc], Comma);
			if (instr.oper1.fmt == 0)
			    strLoc += sprintf(&regStr[strLoc], Reg,
				    instr.oper1.rb);
			else
			    strLoc += sprintf(&regStr[strLoc], Lit,
				    instr.oper2.lit);
			strLoc += sprintf(&regStr[strLoc], Comma);
			break;
		}
	...
	case HW_MTPR:
		...
		if (iprName != NULL)
		{
		    ...
		    strLoc += sprintf(&regStr[strLoc], Comma);
		    strLoc += sprintf(&regStr[strLoc], iprIdx,
			    INDEX_TO_BINARY(instr.hw_mxpr.index));
		    strLoc += sprintf(&regStr[strLoc], Comma);
		    sprintf(&regStr[strLoc], Scbd,
    ...
    sprintf(instrStr, lineFmt, *((u64 *) pcAddr), funcName, regStr,
	    *((u32 *) &instr), (isprint(binFuncStr[0]) ? binFuncStr[0] : '.'),
 ...
 *	None.
 */
void AXP_Dump_Registers(AXP_INSTRUCTION *instr, AXP_REGISTERS *pr,
        AXP_REGISTERS *pf, char *regStr)
{
    int offset;
     ...
     */
    if ((instr->decodedReg.bits.src1 != 0) && (instr->decodedReg.bits.src2 != 0)
	    && (instr->decodedReg.bits.dest != 0))
    {
	offset = sprintf(regStr, Regv,
	        AXP_REG_LETTER(instr->decodedReg.bits.src1), instr->aSrc1,
	        ((instr->decodedReg.bits.src1 & AXP_REG_FP) ?
	                pf[instr->src1].value : pr[instr->src1].value));
	offset += sprintf(&regStr[offset], Comma);
	offset += sprintf(&regStr[offset], Regv,
	        AXP_REG_LETTER(instr->decodedReg.bits.src2), instr->aSrc2,
	        ((instr->decodedReg.bits.src2 & AXP_REG_FP) ?
	                pf[instr->src2].value : pr[instr->src2].value));
	offset += sprintf(&regStr[offset], Comma);
	sprintf(&regStr[offset], Regv,
	        AXP_REG_LETTER(instr->decodedReg.bits.dest), instr->aDest,
	        ((instr->decodedReg.bits.dest & AXP_REG_FP) ?
	                pf[instr->dest].value : pr[instr->dest].value));
    }
    ...
    {
	sprintf(regStr, Regv, AXP_REG_LETTER(instr->decodedReg.bits.src1),
	        instr->aSrc1,
	        ((instr->decodedReg.bits.src1 & AXP_REG_FP) ?
	                pf[instr->src1].value : pr[instr->src1].value));
    }
    ...
    {
	sprintf(regStr, Regv, AXP_REG_LETTER(instr->decodedReg.bits.src2),
	        instr->aSrc2,
	        ((instr->decodedReg.bits.src2 & AXP_REG_FP) ?
	                pf[instr->src2].value : pr[instr->src2].value));
    }
    ...
    {
	sprintf(regStr, Regv, AXP_REG_LETTER(instr->decodedReg.bits.dest),
	        instr->aDest,
	        ((instr->decodedReg.bits.dest & AXP_REG_FP) ?
	                pf[instr->dest].value : pr[instr->dest].value));
    }
