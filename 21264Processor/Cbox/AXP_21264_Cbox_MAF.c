...
#include "21264Processor/Cbox/SystemInterface/AXP_21264_to_System.h"
    ...
    bool shared)
{
  ...
  end1 = cpu->mafBottom;
    ...
    {
  if ((cpu->maf[ii].type != MAFNotInUse) && (cpu->maf[ii].ioReq == false)
      && (cpu->maf[ii].complete == false))
  {
    ...
    if ((paEnd <= pa)
        && ((pa + dataLen)
      <= (cpu->maf[ii].pa + AXP_21264_SIZE_QUAD)))
        maf = &cpu->maf[ii];
      }
  }
    ...
    bool shared)
{
  ...
  end1 = cpu->mafBottom;
    ...
    while ((ii <= end) && (maf == NULL))
    {
  if ((cpu->maf[ii].type != MAFNotInUse) && (cpu->maf[ii].ioReq == true)
      && (cpu->maf[ii].complete == false))
  {
      if (cpu->maf[ii].type == type)
      {
