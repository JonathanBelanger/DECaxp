 ...
 *  Updated to use new directory structure format and clean-up formatting.
 */
...
static crc32cTests crc32cTestCases[] =
{
    ...
    /* Test 7 */
    {
        .buf = (u8 *) "\x1f\x1e\x1d\x1c\x1b\x1a\x19\x18\x17\x16\x15\x14\x13"
                      "\x12\x11\x10\x0f\x0e\x0d\x0c\x0b\x0a\x09\x08\x07\x06"
    ...
    /* Test 8 */
    {
        .buf = (u8 *) "\x01\xc0\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                      "\x00\x00\x00\x14\x00\x00\x00\x00\x00\x04\x00\x00\x00"
                      "\x00\x14\x00\x00\x00\x18\x28\x00\x00\x00\x00\x00\x00"
    ...
    /* Test 13 */
    {
        .buf = (u8 *) "yz{|}~\x7f\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a"
                      "\x8b\x8c\x8d\x8e\x8f\x90\x91\x92\x93\x94\x95\x96\x97"
    ...
    /* Test 14 */
    {
        .buf = (u8 *) "\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab\xac\xad"
                      ...
                      "\xbb\xbc\xbd\xbe\xbf\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7"
                      "\xc8",
        .bufSize = 40,
        .value = 0xba979ad0,
    ...
    /* Test 16 */
    {
        .buf = (u8 *) "\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d"
                      ...
                      "pqrstuvwxyz{|}~\x7f\x80\x81\x82\x83\x84\x85\x86\x87"
...
int main(void)
{
    ...
    char fullPath[AXP_MAX_FILENAME_LEN];
#if 0
    ...
    {
        crcCalc = AXP_Crc32(crc32cTestCases[ii].buf,
    ...
    printf("Test %d: Create a VHDX(v%d) disk in %s with the name of %s of "
           "%llu bytes in size...\n",
           ++ii,
           createParam.ver,
           AXP_TEST_DATA_FILES"/VHDTests",
           diskName,
           createParam.ver_1.maxSize);
    sprintf(fullPath, "%s/VHDTests/%s", AXP_TEST_DATA_FILES, diskName);

    retVal = AXP_VHD_Create(&storageType,
                            fullPath,
                            ACCESS_NONE,
                            NULL,
                            CREATE_NONE,
