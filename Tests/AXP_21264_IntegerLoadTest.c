 ...
 *  Updated to not hard code path to test data files.
 */
...
{
    FILE *fp;
    char *fileNames[] = {"LDA_LDAH_TestData.csv", NULL};
            ...
            while ((feof(fp) == 0) && (pass == true))
            {
                ...
                if (retVal == NoException)
                {
                    if (instr.state == WaitingRetirement)
                    {
                        ...
                        {
                            printf("LDA failed: Rbv = 0x%016llx, disp = "
                                   "0x%04llx, Rav = 0x%016llx (expected: "
                                   "0x%016llx)\n",
                                   instr.src1v.r.uq,
                                   instr.displacement,
                                   instr.destv.r.uq,
                ...
                if (pass == true)
                {
                    ...
                    instr.state = Executing;
                    testCnt++;
                    /* Call the LDAH - Load Address High instruction. */
                    retVal = AXP_LDAH(cpu, &instr);
                    if (retVal == NoException)
                    {
                        if (instr.state == WaitingRetirement)
                        {
                            ...
                            {
                                printf("LDAH failed: Rbv = 0x%016llx, disp = "
                                       "0x%04llx, Rav = 0x%016llx (expected: "
                                       "0x%016llx)\n",
                                       instr.src1v.r.uq,
                                       instr.displacement,
