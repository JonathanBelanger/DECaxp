 ...
 *  LDx I/O         X       X   X       X       RdBytes, RdLWs, or RdQWs based
 *                                              on size.
 *  Istream Memory  1       X   X       X       Dcache hit, Istream serviced
 *                                              from Dcache.
 *  Istream Memory  0       X   1       X       Bcache hit, Istream serviced
 *                                              from Bcache.
 *  Istream Memory  0       X   0       X       Miss, generate RdBlkI command.
 *  STx Memory      1       1   X       X       Store Dcache hit and writable,
 *                                              done.
 *  STx Memory      1       0   X       X       Store hit and not writable, set
 *                                              dirty flow (note 1).
 *  STx Memory      0       X   1       1       Store Bcache hit and writable,
 *                                               done.
 *  STx Memory      0       X   1       0       Store hit and not writable,
 *                                              set-dirty flow (note 1).
 *  STx Memory      0       X   0       X       Miss, generate RdBlkMod commnd.
 *  STx I/O         X       X   X       X       WrBytes, WrLWs, or WrQWs based
 *                                              on size.
 *  STx_C Memory    0       X   X       X       Fail STx_C.
 *  STx_C Memory    1       0   X       X       STx_C hit and not writable,
 *                                              set-dirty flow (note 1).
 *  STx_C I/O       X       X   X       X       Always succeed and WrQws or
 *                                              WrLws are generated, based on
 *                                              the size.
 ...
 *                                              command (note 2).
 *  WH64 I/O        X       X   X       X       NOP the instruction. WH64 is
 *                                              UNDEFINED for I/O space.
 *  ECB Memory      X       X   X       X       Generate evict command (note 3).
 *  ECB I/O         X       X   X       X       NOP the instruction. ECB
 *                                              instruction is UNDEFINED for
 *                                              I/O space.
 *  MB/WMB          X       X   X       X       Generate MB command (note 4).
 *  -----------     -----   --- -----   ---     ------------------------------
 *  Table 4-1 Notes:
...
#define AXP_CACHE_MISS(_status)                                             \
    ((_status) == AXP_21264_CACHE_MISS)
#define AXP_CACHE_HIT(_status)                                              \
  (((_status) & AXP_21264_CACHE_HIT) == AXP_21264_CACHE_HIT)
#define AXP_CACHE_CLEAN(_status)                                            \
    ((_status) == AXP_21264_CACHE_CLEAN)
#define AXP_CACHE_CLEAN_SHARED(_status)                                     \
    ((_status) == AXP_21264_CACHE_CLEAN_SHARED)
#define AXP_CACHE_DIRTY(_status)                                            \
    ((_status) == AXP_21264_CACHE_DIRTY_)
#define AXP_CACHE_DIRTY_SHARED(_status)                                     \
    ((_status) == AXP_21264_CACHE_DIRTY_SHARED)
