 ...
 */
u32 AXP_VHD_ValidateCreate(AXP_VHD_STORAGE_TYPE *storageType,
                           char *path,
                           ...
                           u32 *deviceID)
{
    u64 minDisk = 0, maxDisk = 0;
    u32 defBlk = 0, minBlk = 0, maxBlk = 0;
    u32 defSector = 0, minSector = 0, maxSector = 0;
        ...
        (path != NULL))
    {
        ...
        if (retVal == AXP_VHD_SUCCESS)
        {
             ...
             */
            if (((param->ver != CREATE_VER_1) &&
                 (param->ver != CREATE_VER_2)) ||
                ((param->ver == CREATE_VER_2) &&
                 (accessMask != ACCESS_NONE)) ||
                (flags > CREATE_FULL_PHYSICAL_ALLOCATION) ||
                ((accessMask & ~ACCESS_ALL) != 0) ||
                 (((*blkSize <= minBlk) ||
                   (*blkSize >= maxBlk)) ||
                  (IS_POWER_OF_2(*blkSize) == false)) ||
                 ((*sectorSize != minSector) &&
                  (*sectorSize != maxSector)) ||
                 ((*diskSize < minDisk) ||
                  (*diskSize > maxDisk) ||
 ...
 *                      parameters was detected.
 */
u32 AXP_VHD_ValidateOpen(AXP_VHD_STORAGE_TYPE *storageType,
                         char *path,
                         AXP_VHD_ACCESS_MASK accessMask,
                         ...
                         u32 *deviceID)
{
     ...
     * ever called from within the tool.
     */
    if ((storageType != NULL) && (handle != NULL) && (path != NULL))
    {
         ...
         */
        if (((param != NULL) &&
             (param->ver != OPEN_VER_1)) ||
            ((accessMask & ~ACCESS_ALL) != 0) ||
             ((flags != OPEN_NO_PARENTS) &&
              (flags != OPEN_BLANK_FILE)))
        {
            retVal = AXP_VHD_INV_PARAM;
        }
        else
 ...
 */
u32 AXP_VHD_ValidateRead(AXP_VHD_HANDLE handle,
                         u64 lba,
                         u32 sectorsRead,
                         u32 *deviceID)
{
 ...
 *  TODO: Look at other potential valid error return values.
 */
u32 AXP_VHD_ValidateWrite(AXP_VHD_HANDLE handle,
                          u64 lba,
                          u32 sectorsWritten,
                          u32 *deviceID)
{
    ...
    if (AXP_UTL_BUFF)
    {
        ...
        if (AXP_ReturnType_Block(vhdx) == AXP_VHDX_BLK)
        {
            if (vhdx->fp != NULL)
            {
                 ...
                 */
                readRet = AXP_ReadFromOffset(vhdx->fp,
                                            buffer,
                                            &retLen,
                                            AXP_VHDX_HDR_LOC);
                if (readRet == true)
                {
                    ...
                    AXP_TraceWrite("\t3.1.1 File Type Identifier: @ 0x%016llx",
                                   AXP_VHDX_HDR_LOC);
                ...
                if (readRet == true)
                {
                    for (ii = 0; ((ii < 2) && (readRet == true)); ii++)
                    {
                        ...
                        if (readRet == true)
                        {
                            ...
                            AXP_TraceWrite("\t3.1.2 Headers %d: @ 0x%016llx",
                                           ii+1,
                                           offset);
                            AXP_TraceWrite("\t\tSignature: %.4s",
                                           (char *) &hdr->sig);
                            AXP_TraceWrite("\t\tChecksum: 0x%08x",
                                           hdr->checkSum);
                            AXP_TraceWrite("\t\tSequenceNumber: 0x%016x (%llu)",
                                           hdr->seqNum,
                                           hdr->seqNum);
                            AXP_Convert_From(GUID,
                                             &hdr->fileWriteGuid,
                                             &hdr->fileWriteGuid);
                            data4a = (u16 *) &hdr->fileWriteGuid.data4;
                            AXP_TraceWrite("\t\tFileWriteGuid: %08x-%04x-%04x-"
                                           "%04x-%04x%04x%04x",
                            ...
                            AXP_TraceWrite("\t\tDataWriteGuid: %08x-%04x-%04x-"
                                           "%04x-%04x%04x%04x",
                            ...
                            AXP_TraceWrite("\t\tLogGuid: %08x-%04x-%04x-%04x-"
                                           "%04x%04x%04x",
                                           hdr->logGuid.data1,
                                           hdr->logGuid.data2,
                                           hdr->logGuid.data3,
                            ...
                            AXP_TraceWrite("\t\tLogOffset: 0x%016llx (%llu)",
                                           hdr->logOff,
                                           logOffset);
                        }
                    }
                ...
                if (readRet == true)
                {
                    ...
                    for (ii = 0; ((ii < 2) && (readRet == true)); ii++)
                    {
                        ...
                        if (readRet == true)
                        {
                            ...
                            AXP_TraceWrite("\t3.1.3 Region Table %d: @ "
                                           "0x%016llx",
                                           ii+1,
                                           offset);
                            AXP_TraceWrite("\t\tSignature: %.4s",
                                 ...
                                 jj++)
                            {
                                ...
                                AXP_TraceWrite("\t\t\tGuid: %08x-%04x-%04x-"
                                               "%04x-%04x%04x%04x",
                                               ent->guid.data1,
                                               ent->guid.data2,
                                               ent->guid.data3,
                                ...
                                localOffset = (batOffset == 0) ?
                                        metadataOffset :
                                        batOffset;
                                AXP_TraceWrite("\t\t\tOffset: %016llx (%llu)",
                                               ent->fileOff,
                                               localOffset);
            ...
            {
            AXP_TraceWrite(">>>>> No file pointer found in VHD Handle used"
                           " to trace VHD File. <<<<<");
            }
        }
        else
